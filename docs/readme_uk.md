...
<a id="quick-start"></a>
**Віддаєте перевагу інтерфейсу MCP-сервера?** Підключіть [cursor-history-mcp](https://github.com/S2thend/cursor-history-mcp#quick-start), щоб надати доступ до історії через інструменти MCP. Агенти також можуть безпосередньо викликати CLI або API Node.js цього проєкту — виберіть інтерфейс під свій робочий процес. Пакети випускаються незалежно; перевірте [сумісність сховищ і відомості про випуски MCP](https://github.com/S2thend/cursor-history-mcp#compatibility).
...
[Порівняння](#comparison) · [Встановлення](#installation) · [Використання](#usage) · [Приклади виводу](../README.md#example-output) · [API бібліотеки](#library-api) · [План розвитку](#roadmap) · [Сумісність](#compatibility)

<a id="comparison"></a>

## Чому варто обрати cursor-history?

Оберіть `cursor-history`, якщо хочете знаходити, переглядати, зберігати й повторно використовувати історію Cursor через єдиний CLI та API Node.js.

- **Кілька джерел Cursor, один інтерфейс** — Читайте підтримувані джерела Composer, транскрипти Agent, Store / CLI та ACP.
- **Від пошуку до збереження** — Шукайте в тексті розмов у різних робочих просторах, переглядайте доступні diff і дії інструментів та експортуйте в Markdown / JSON. Створюйте резервні копії даних Composer і відновлюйте їх або перевіряйте перенесення придатних сеансів Composer через `--dry-run`.
- **Інтеграція у ваш робочий процес** — Використовуйте API Node.js у власних інструментах, підключайте окремий [супутній MCP-проєкт](https://github.com/S2thend/cursor-history-mcp#compatibility) або за [налаштуваннями WSL](#storage) читайте дані Store з Windows чи зсередини WSL.

| Критерій | **cursor-history (цей проєкт)** | [deja-vu](https://github.com/vshulcz/deja-vu) | [cursaves](https://github.com/Callum-Ward/cursaves) | [johnlindquist/cursor-history](https://github.com/johnlindquist/cursor-history) |
|---|---|---|---|---|
| Призначення | Перегляд і керування історією Cursor; CLI + API Node.js | Пам’ять між агентами | Синхронізація Git / S3 | Перегляд, експорт, буфер обміну |
| Задокументовані джерела Cursor | Composer, транскрипти Agent, Store / CLI, ACP | SQLite IDE, транскрипти CLI¹ | SQLite робочих просторів/глобальна | Не задокументовано |
| Пошук | Текст розмов у різних робочих просторах | Індекс між агентами | Не задокументовано | Нечіткий пошук назв |
| Збереження й перенесення | Копіювання/відновлення баз Composer; перенесення придатних сеансів Composer | Синхронізація/передача пам’яті | Відновлення знімків; копіювання між просторами | Не задокументовано |

Документацію перевірено **2026-09-09** за README з посилань та ¹ [описом формату Cursor у deja-vu](https://github.com/vshulcz/deja-vu/blob/main/docs/registry/cursor.md); порівняльних тестів запуску не проводили. «Не задокументовано» означає, що перевірені джерела не описують можливість, а не підтверджує її відсутність.

Наші резервне копіювання й відновлення охоплюють лише бази Composer. Перенесення підтримує придатні сеанси Composer, виключаючи сеанси лише зі Store та сеанси з об’єднаних джерел. Доступні для читання сеанси Store / ACP і транскрипти можна експортувати, але ці файли не є резервними архівами для відновлення. Див. [контракт сумісності](./compatibility.md).
